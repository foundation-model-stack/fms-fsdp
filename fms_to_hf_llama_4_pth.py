# To convert LAST model stored in ./pth (rather than ./checkpoints) to HF-compatible model
# TODO: DQA NOT RUNNABLE YET!
...
def main(
    ...
    print(f"is_old_fms: {is_old_fms}")
    print(f"load_path: {load_path}")
    print(f"save_path: {save_path}")
    print(f"tokenizer_name_or_path: {tokenizer_name_or_path}")
    print(f"======================================================\n")
    
    
    
    
    model = torch.load(f"{load_path}consolidated.00.pth")
    state_dict = torch.load()["model_state"]
    for key in state_dict.keys():
        state_dict[key[10:]] = state.pop(key)
    ...
    state_dict = torch.load(f"{load_path}consolidated.00.pth")
