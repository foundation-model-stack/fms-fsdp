"""
Call this file to convert model trained with /proj/data/fsdp/train_env # ab.study env
Not /proj/data-eng/granite_docling/conda-envs/fms-fsdp-env
...
def convert_to_hf(model: LLaMA, model_variant, is_old_fms) -> LlamaForCausalLM:
    ...
    if "llama165m_granite4tiktoken" in model_variant:
        hf_config.bos_token_id = 100257
        hf_config.eos_token_id = 100257
    ...
    with torch.no_grad():
        ...
        for oss_hf_layer in oss_hf_model.model.layers:
            ...
            #== compared to version in `main` branch on 2025 May
            #== use else part for attn, while if part for mlp:
            #== https://github.com/foundation-model-stack/fms-fsdp/blob/503da7ede354e1ffdabc80fae8bbd211cb2174c8/fms_to_hf_llama.py#L59
            # self attn
            q, k, v = torch.split(
