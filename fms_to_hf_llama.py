'''
To convert all checkpoints from an experiment:
bsub -Is -M 512G -gpu "num=2/task:mode=exclusive_process" /bin/bash
cd /proj/data-eng/granite_docling/launch_scripts

rm -rf /proj/data-eng/granite_docling/experiments/00-test/hf_model/step_100000_ckp 
./convert_llama165m_2_hf_format.sh /proj/data-eng/granite_docling/experiments/00-test llama165m_granite4tiktoken /proj/data-eng/tokenizers/granite4tiktoken

./convert_llama165m_2_hf_format.sh /proj/data-eng/granite_docling/experiments/llama165m_granite4tiktoken-2T-pdbs2-8node llama165m_granite4tiktoken /proj/data-eng/tokenizers/granite4tiktoken
'''
import fire
import torch
    ...
    oss_hf_model = LlamaForCausalLM(
        LlamaConfig(
            ...
            * (
            ...
            tie_word_embeddings=hf_config.tie_heads
        )
    )
